...
/*------------------------------------------------------------------------------
	...
	eBookCMS 1.68
	Stable Release date: August 16, 2015
...
// SHORT SESSION_ID
	...
	if (isset($_SESSION[HURI]['site']) && $_SESSION[HURI]['site'] != HURI) {
		session_destroy();
		$_SESSION[HURI]['site'] = HURI;
	}
...
// REQUEST PAGE OR HOME
	if (isset($uri[0]) && ($uri[0] == 'page' || $uri[0] == 'show')) {
		...
		if ($result = db() -> prepare($query)) {
			...
			if (!$acontent) {unset($acontent, $result); set_error();}
			if ($acontent['ptype'] == 0) {
...
function get_header() {global $get, $charset, $acontent; $lfeed = PHP_EOL.chr(9);
	...
	echo $lfeed.'<script type = "text/javascript" src = "js/ebookcms.js"></script>'.chr(13);
}
...
function set_error($error = '404') {
	echo '<meta http-equiv="refresh" content="0; url='.pre_seao('error', $error, false, true).'">';
}

// SHOW ERROR INFORMATION
function show_error($error = '401') {
	echo '<h1>'.t('error').'</h1>';
	switch ($error) {
		case '401': echo '<p>'.t('unauthorized').'</p>'; break;
		case '404': header('HTTP/1.1 404 Not Found'); echo '<p>'.t('bad_url').'</p>'; break;
	}
}
...
function comment($page_id, $ptype, $cplugin) { global $get, $uri, $acontent;
	if (!isset($_SESSION[HURI][erukka('try_hack')]) && !$_POST) {
		...
		if (!empty($error)) {
			...
			$query = "INSERT INTO ".PREFIX."comments (plug_id, pcontent_id, ptype, reply_id, user_id, user_ip, name, avatar_email, url, comment, date_reg, approved, key_security, delkey)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
...
function submit_register() { global $get, $acontent;
	if ($_POST && checkSpamRSA()) { $error = '';
		...
		if (!empty($error)) {
			...
			$query = "INSERT INTO ".PREFIX."users (user_active, user_ip, username, realname, avatar, password, gmember, emailreg, email, website, regdate, vcode, akey, key_security, del_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
			if ($result = db() -> prepare($query)) {
...
function pmessage($ptype = 0, $author = 1, $gmember = 1) { global $get;
	if ($_POST && checkSpamRSA()) {
		...
		$unread = 1;
		$ks = genKey('key_security'); $delkey = genKey('del_key');
		$query = "INSERT INTO ".PREFIX."pmessages (ptype, user_id, suser_id, gmember, page_id, lang_id, date_sent, name, email, subject, message, unread, key_security, delkey) 
			VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		if ($result = db() -> prepare($query)) {
...
// UPDATE TIME LOGIN
if (LOGGED) {
	$id = $_SESSION[HURI]['user_id']; $now = strtotime("now"); $time_login = date('Y-m-d H:i:s', strtotime('+'.TIME_OUT.' seconds'));
	$ip = getInfo('YOUR_IP') == '::1' ? '127.0.0.1' : getInfo('YOUR_IP');
