...
function verify_admin($table, $id) { global $get; $hereited = false;
	...
	switch ($table) {
		case 'pages': case 'cpages': case 'products' : $hereited = true;
...
# **************************************************************************************************************
	...
	$here = '<a href="'.pre_seao($uadmin, $gadmin, false, true).pre_seao('action', 'new').$tb.$lang.$ukey.'">'.t('here').'</a>';
...
# **************************************************************************************************************
	...
	if (($_POST && isset($_POST[erukka('submit')])) || (isset($_POST['adm']) && $_POST['adm'] == 'config')) {
		...
		if (!empty($reserved)) {
			...
			$redirect = pre_seao($uadmin, $gadmin, false, true).pre_seao('action', $last_action).$tid.$uid.$utks.$pd.$ukey.$pnn;
		...
		if ($parent) {echo '<meta http-equiv="refresh" content="1; url='.pre_seao('page', $parent, false, true).$pnn.'">'; return;}
			...
			switch ($field[0]) {
				...
				case "create_tab" : 
					...
					for ($j=0; $j<count($tfields); $j++){ $index = $j+1;
						$class1 = $j == 0 ? ' class="current"' : '';
						$class2 = $j == 0 ? ' class="active"' : '';
				...
				case 'add_tab': 
					$class = $field[1] == 1 ? 'class = "tab_content active"' : 'class = "tab_content"';
				...
				case 'input_xtext': case 'input_float': case 'input_tags': case 'input_int': //case 'input_dxtext':
